...
#
#   1. Stages base VHDX + per-VM seed ISO via lab/stage-core-base.sh
#   2. Copies host-side helper scripts (incl. New-CoreTestVM.ps1) to
...
#   6. scp's prepare-image.sh + core-sconfig.sh to /tmp on the VM and
#      runs prepare-image.sh
#   7. Snapshots the powered-off VM as `deploy-master` (host-agnostic)
#   8. Boots once to fire core-firstboot, snapshots as `golden-image`
#
...
#
# Defaults track lab/core.env (core-1 / 10.10.10.40 / debadmin /
# MAC 00155D0A0A28); override via env vars or flags.
#
# The lab reuses the samba-addc-appliance lab environment: same router1,
# same Lab-NAT. The blank does not need WS2025-DC1, doesn't join a
# domain, doesn't touch any of the product-specific lab pieces.
#===============================================================================
set -euo pipefail
...
# Defaults intentionally match lab/core.env so a flagless invocation
# produces core-1 ready for the existing scenarios.
VM_NAME="${VM_NAME:-core-1}"
VM_IP="${VM_IP:-10.10.10.40}"
...
FORCE=0
# core-firstboot lands in a follow-up phase. Until then the build stops
# at the host-agnostic deploy-master snapshot, which is what gets
# exported and tested in different environments. Pass --with-firstboot
# (no-op today) once the service is in place.
WITH_FIRSTBOOT=0
...
Flags:
  ...
  -m, --mac MAC          Pinned NIC MAC, no separators
                         (default: $STATIC_MAC)
  ...
  -f, --force            Remove the VM and its diff VHDX first if present
      --with-firstboot   After deploy-master, boot once and wait for
                         ...
                         golden-image. Off by default — core-firstboot
                         is not yet implemented; default behavior stops
                         at deploy-master, which is the artifact that
                         gets exported and tested cross-environment.
  -h, --help             Show this
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --with-firstboot)  WITH_FIRSTBOOT=1; shift ;;
        -h|--help)         usage; exit 0 ;;
...
ls -la "$STAGE_DIR_MAC/New-CoreTestVM.ps1"

step "3. create Hyper-V VM $VM_NAME"
ssh_host "pwsh -File ${STAGE_DIR_HOST}\\New-CoreTestVM.ps1 \
            -VMName '$VM_NAME' \
...
ssh_vm 'hostname; ip -4 addr show | grep -E "inet " | head -3; \
        test -f /var/log/appliance-core-base-ready.marker && cat /var/log/appliance-core-base-ready.marker'

step "5. push appliance scripts and lib/ to the VM"
# prepare-image.sh's vendoring step looks at /tmp/lib/ first, so the
# whole lib/ directory must travel alongside the prep script.
scp -J "${HV_USER}@${HV_HOST}" \
    -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null \
    "$REPO_DIR/prepare-image.sh" "$REPO_DIR/core-sconfig.sh" \
    "${VM_USER}@${VM_IP}:/tmp/"
...
# core-firstboot has NOT yet fired. The disk image at this point can
...
if [[ $WITH_FIRSTBOOT -ne 1 ]]; then
    echo
    echo "Default behavior: stopping after deploy-master (firstboot deferred)."
    ...
    echo "  lab/build-fresh-base.sh --with-firstboot   # once core-firstboot lands"
    exit 0
fi

step "8. boot once to fire core-firstboot (Hyper-V tailoring)"
ssh_host "Start-VM -Name '$VM_NAME'"

say "wait for core-firstboot.done marker (up to 120s)"
done_marker=0
for _ in $(seq 1 60); do
    if ssh -o ConnectTimeout=3 -o BatchMode=yes \
           -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null \
           -J "${HV_USER}@${HV_HOST}" "${VM_USER}@${VM_IP}" \
           'test -f /var/lib/core-firstboot.done' 2>/dev/null; then
        done_marker=1
        break
    fi
    sleep 2
done
[[ $done_marker -eq 1 ]] || { say "core-firstboot.done never appeared"; exit 1; }

# Show what firstboot did so it's in the build log.
ssh_vm 'sudo cat /var/log/core-firstboot.log 2>/dev/null | tail -40' || true
