#!/usr/bin/env bash
#===============================================================================
# lab/export-deploy-master.sh — release-time export of the appliance-core blank
...
# Final layout:
#
#   dist/appliance-core-vYYYY.MM.DD/
#     appliance-core-vYYYY.MM.DD.vhdx     # Hyper-V users (lossless)
#     appliance-core-vYYYY.MM.DD.qcow2    # KVM/Proxmox/Nutanix/OpenStack
#     appliance-core-vYYYY.MM.DD.vmdk     # streamOptimized; same content
#                                              #  as the .vmdk inside the .ova
#     appliance-core-vYYYY.MM.DD.ova      # VMware/VirtualBox
#     SHA256SUMS
#
...
# The blank appliance is single-NIC by design, so the import is
# straightforward in every supported hypervisor.
#===============================================================================
set -euo pipefail
...
recompute_paths() {
    ARTIFACT_BASE="appliance-core-v${VERSION}"
...
# NIC (vmxnet3, NAT) — the blank is single-NIC by design.
cat > "$VMX_TMP" <<VMXEOF
.encoding = "UTF-8"
config.version = "8"
virtualHW.version = "19"
displayName = "${ARTIFACT_BASE}"
annotation = "Appliance Core (blank) | host-agnostic deploy master | exported $(date -u +%Y-%m-%dT%H:%M:%SZ)"
guestOS = "debian12-64"
firmware = "efi"
