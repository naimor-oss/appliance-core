#!/usr/bin/env bash
#===============================================================================
# stage-core-base.sh - Mac-side staging for the appliance-core blank
#
# Produces two files on the ISO share (/Volumes/ISO by default,
# = D:\ISO\ on the Hyper-V host):
#
#   debian-13-appliance-core-base.vhdx  ~1.2 GB - Debian genericcloud qcow2 converted.
#                                  Built once, reused for every test VM you make.
...
# the link between dnsmasq (which hands out the reserved 10.10.10.40 lease
# when it sees that MAC) and the cloud-init network config (which DHCPs
# the matching interface).
# cloud-init; core-init's role wizard sets its static IP later.
#
...
#
# Usage:
#   lab/stage-core-base.sh                                # core-1 / lab.test
#   lab/stage-core-base.sh -n core-2 -i 10.10.10.31 -m 00155D0A0A1F
#   lab/stage-core-base.sh -u debadmin -k ~/.ssh/lab.pub
#===============================================================================
set -euo pipefail
...
# Domain-NIC MAC pinned by lab/hyperv/New-CoreTestVM.ps1. The default
# 00:15:5D:0A:0A:28 maps to dnsmasq reservation 10.10.10.40 (core-1).
# Last-octet pattern: 0A14=20 samba-dc1, 0A1E=30 core-1, etc.
DOMAIN_MAC='00155D0A0A28'
DOMAIN_IP='10.10.10.40'

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SEED_SRC="$SCRIPT_DIR/templates/cloud-init"
KEYS_DIR="$SCRIPT_DIR/keys"

# Single-pubkey override; if -k FILE is passed we ignore $KEYS_DIR.
SSH_PUBKEY_FILE=""
ALLOW_NO_KEYS=0
...
Options:
  ...
  -k, --pubkey FILE       Single SSH pubkey file ( overrides
                          lab/keys/ if used).
...
for tpl in user-data-core.tpl meta-data.tpl network-config.tpl; do
    [[ -f "$SEED_SRC/$tpl" ]] || die "template missing: $SEED_SRC/$tpl"
done

# Normalize MAC: accept 00155D0A0A28 or 00:15:5D:0A:0A:28, produce both.
...
# becomes a YAML list entry indented to match user-data-core.tpl's
# six-space indent.
SSH_KEYS_BLOCK=""
KEY_SOURCES=""
add_key_line() {
...
#---- 1. base VHDX (shared across all core test VMs) ----
CACHE_QCOW2="$STAGE_DIR/debian-13-genericcloud-${ARCH}.qcow2"
OUT_VHDX="$STAGE_DIR/debian-13-appliance-core-base.vhdx"
...
if [[ ! -f "$OUT_VHDX" ]]; then
    ...
    tmp_qcow=$(mktemp /tmp/core-base-XXXX.qcow2)
    tmp_vhdx=$(mktemp /tmp/core-base-XXXX.vhdx)
...
SEED_BUILD_DIR=$(mktemp -d /tmp/seed-core-XXXX)
...
# user-data-core.tpl has a multi-line @@SSH_KEYS_BLOCK@@ placeholder
...
awk '
    ...
    { print }
' "$SEED_SRC/user-data-core.tpl" | substitute /dev/stdin > "$SEED_BUILD_DIR/user-data"
...
echo "  ssh <host-user>@<hyper-v-host> 'pwsh -File D:\\ISO\\lab-scripts\\New-CoreTestVM.ps1 \\"
